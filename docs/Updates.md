...
## 2. Mixtral-8x7b는 완전히 제외됨

- **계획**: 4개 모델(qwen2.5-32b, gemma4-31b, llama3.3-70b, mixtral-8x7b) x
  각 모델의 quant level = 총 14개 combo.
- **실제**: Mixtral의 레이어당 8개 MoE expert가 개별 `nn.Linear` 모듈이 아니라
  fused 3D 텐서(`gate_up_proj`/`down_proj`)로 저장되어 있어서, bitsandbytes가
  이걸 전혀 양자화할 수 없음 — 그래서 Mixtral 파라미터의 약 96%가
  `device_map`, CPU offload, 커스텀 streaming quantizer 어떤 방법을 써도
  전혀 손대지지 않음 (세 방법 모두 검토했지만, 어느 것도 이 구조적 한계를
  해결하지 못함). `configs/run_matrix.yaml`의 `known_risk` 필드에 알려진
  한계로 문서화하고 보류함. 최종 실행은 **14개 중 11개 combo**만 커버함
  (남은 3개 모델 x 각 3개 quant level).
...
- `spike_test_mixtral_bnb.py` / `spike_test_gemma4_bnb.py`는 더 이상 raw
  ...
  PASS/FAIL 결과가 실제 Phase 3의 동작을 그대로 대변함.
